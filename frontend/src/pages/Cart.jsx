...
export default function Cart() {
  ...
  const getCompositionText = (customization) => {
    ...
    if (customization.filling?.length) {
      parts.push(
        `начинка: ${customization.filling.map((f) => f.name).join(", ")}`,
      );
    }
    ...
    return (
      <main className="border-t pt-14">
        ...
        <div className="py-20 text-center">
          <p className="text-gray-700">Корзина пуста</p>
          <button
      ...
      <div>
        {cartData.map((item, index) => (
          ...
          >
            <div className="flex items-start gap-6">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-700">
                  {currency}
                  {item.totalPrice} за шт.
                </p>
                <p
                  className="mt-1 text-xs text-gray-700 max-w-[200px] sm:max-w-xs"
            ...
            >
              <img src={assets.bin_icon} alt="" width={20} height={20} />
            </button>
          </div>
        ))}
      </div>
