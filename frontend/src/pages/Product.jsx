...
export default function Product() {
  ...
  // Для форм и основ проверяем выбран ли размер
  if ((productData.category === "form" || productData.category === "base") && !selectedSize) {
    toast.error("Пожалуйста, выберите вес");
    return;
  }

  // Получаем текущие сохранённые опции
  const saved = localStorage.getItem("preselectedOptions");
  let preselectedOptions = saved ? JSON.parse(saved) : [];

  // НОВАЯ ЛОГИКА: проверяем, не выбрана ли уже такая опция
  const existingIndex = preselectedOptions.findIndex(
    (opt) => opt.category === productData.category && opt.optionId === productData._id
  );

  // Если опция уже есть — НЕ ДОБАВЛЯЕМ повторно
  if (existingIndex === -1) {
    ...
    // Если уже есть, обновляем размер (если нужно)
    ...
    <div className="pt-10 transition-opacity duration-500 ease-in border-t-2 opacity-100">
      <div className="flex flex-col gap-12 sm:gap-12 sm:flex-row">
        ...
        <div className="flex flex-col-reverse flex-1 gap-3 sm:flex-row">
          ...
          <div className="w-full sm:w-[80%]">
            <img className="w-full h-auto rounded-lg" src={`${backendUrl}${image}`} alt="" />
          </div>
        </div>
        ...
        <div className="flex-1">
          ...
          {/* Выбор веса (только для форм и основ) */}
          {(productData.category === "form" || productData.category === "base") && productData.sizes?.length > 0 && (
      ...
      <div className="mt-20">
        ...
        <div className="flex flex-col gap-4 px-6 py-6 text-sm text-gray-500 border rounded-bl rounded-br">
          ...
          <p className="mt-2">
            Эта опция идеально подходит для создания уникального шоколада ручной работы.
            Вы можете комбинировать её с другими ингредиентами и украшениями в нашем
            <button onClick={() => navigate("/collection")} className="ml-1 text-gray-800 hover:underline">
              конструкторе
            </button>.
          </p>
        </div>
